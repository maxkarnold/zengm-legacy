...
const CustomRosters: React.FC = () => {
    ...
    return (
        <div>
            ...
            <p>
                By default, draft classes are filled with randomly-generated players. If you're using a custom League File,
                that can include up to 3 draft classes of pre-defined players.
            </p>
            <p>
                Additionally, it is possible to overwrite a draft class in any current league with pre-defined players.
                To do this, go to Players {'>'} Draft {'>'} Draft Scouting, click "Customize", and upload a Draft Class File for whichever year you want.
                When you do this, it will delete all of the current prospects for that draft class, insert players from the uploaded file,
                and (if necessary) randomly generate some new players to fill out the draft class if the file contains less than 70 players.
            </p>
            <p>
                Like with the custom League Files, there are two places to get Draft Class Files: download one or make your own.
                Again like League Files, you might find some to download <a href="https://www.reddit.com/r/ZenGMLOl/">on Reddit</a>.
                You can even use a League File as a Draft Class File - it will just take whoever is defined in the first draft and ignore everything else in the League File.
            </p>
            <p>
                Just make sure to give every player a "tid" of -2. You can define as few or as many players as you want;
                if you define less than needed number, then randomly generated players will fill out the draft class when people use it.
            </p>
            ...
            <p>
                <code>season</code> is the current season. <code>startingSeason</code> is the first season played in this league.
                <code>gracePeriodEnd</code> is the first season you can get fired after. If you set one of these, you should set all of them.
                You should also specify seasons for any stats, ratings, and attributes for your players and teams.
            </p>
            <p>
                However, there is a shortcut to make things easier. Move <code>startingSeason</code> up to the root of the League File
                (at the same level as <code>gameAttributes</code>) and it will be used to automatically set all of the season variables everywhere.
                Or just leave <code>startingSeason</code> and all the other seasons out completely and the default value will be used.
            </p>

            <h3>Champions</h3>
            <p>
                <code>champions</code> contains all the champion basic data, such as counter, synergy, and early/mid/late data.
                In general, this is difficult to add to because counter and synergy data needs to be put in for each champion combination.
                However, if you just want to adjust how strong each champion is then you use the far easier <code>championPatch</code>
            </p>

            <h3>Champion Patch Data</h3>
            <p>
                <code>championPatch</code> is the current strength of each champion in each role. It is relatively easy to change in game or by uploading a file.
                All champion and role combinations are given a win rate (0 to 1). So if you change that win rate to 1 the champion should in theory never lose and at 0 would never win.
                For the most part win rates are between .45 and .55.
                Outside that range would be considered particularly weak or strong
            </p>
        </div>
    );
};

export default CustomRosters;
